...
class AxisAlignedTargetAssigner(object):
    ...
    def assign_targets(self, all_anchors, gt_boxes_with_classes):
        ...
        gt_classes = gt_boxes_with_classes[:, :, -1] # 类别信息
        gt_boxes = gt_boxes_with_classes[:, :, :-1] # 检测框参数
        for k in range(batch_size):
            ...
            target_list = []
            for anchor_class_name, anchors in zip(self.anchor_class_names, all_anchors): # 逐类别匹配
                if cur_gt_classes.shape[0] > 1:
                    mask = torch.from_numpy(self.class_names[cur_gt_classes.cpu() - 1] == anchor_class_name) # 取出当前类别的gt
                else:
    ...
    def assign_targets_single(self, anchors,
        ...
        if len(gt_boxes) > 0 and anchors.shape[0] > 0:
            # pdb.set_trace() 计算anchor和gt之间的两两IoU
            anchor_by_gt_overlap = iou3d_nms_utils.boxes_iou3d_gpu(anchors[:, 0:7], gt_boxes[:, 0:7]) \
                if self.match_height else box_utils.boxes3d_nearest_bev_iou(anchors[:, 0:7], gt_boxes[:, 0:7])

            # anchor_to_gt_argmax = torch.from_numpy(anchor_by_gt_overlap.cpu().numpy().argmax(axis=1)).cuda()
            anchor_to_gt_argmax = torch.argmax(anchor_by_gt_overlap, dim=1) # 取出IoU最大的gt索引
            anchor_to_gt_max = anchor_by_gt_overlap[
                torch.arange(num_anchors, device=anchors.device), anchor_to_gt_argmax
            ] # 根据行索引、列索引获取匹配最大的IoU

            # gt_to_anchor_argmax = torch.from_numpy(anchor_by_gt_overlap.cpu().numpy().argmax(axis=0)).cuda()
            gt_to_anchor_argmax = torch.argmax(anchor_by_gt_overlap, dim=0) # 取出IoU最大的anchor索引
            gt_to_anchor_max = anchor_by_gt_overlap[gt_to_anchor_argmax, torch.arange(num_gt, device=anchors.device)] # 根据行索引、列索引获取匹配最大的IoU
            empty_gt_mask = gt_to_anchor_max == 0 # 将最大IoU为0的gt设为空gt
            gt_to_anchor_max[empty_gt_mask] = -1 # 将空gt的最大匹配IoU设为-1

            anchors_with_max_overlap = (anchor_by_gt_overlap == gt_to_anchor_max).nonzero()[:, 0] # 取出与gt最大IoU相等的anchor索引
            gt_inds_force = anchor_to_gt_argmax[anchors_with_max_overlap] # 取出对应gt的类别
            labels[anchors_with_max_overlap] = gt_classes[gt_inds_force] # 将对应gt类别设为该anchor的标签
            gt_ids[anchors_with_max_overlap] = gt_inds_force.int() # 设置对应的gt索引

            pos_inds = anchor_to_gt_max >= matched_threshold # 将正样本设为True
            gt_inds_over_thresh = anchor_to_gt_argmax[pos_inds] # 获取正样本的gt索引
            labels[pos_inds] = gt_classes[gt_inds_over_thresh] # 取出正样本对应的gt类别，作为正样本anchor标签
            gt_ids[pos_inds] = gt_inds_over_thresh.int() # 设置对应的gt索引设
            bg_inds = (anchor_to_gt_max < unmatched_threshold).nonzero()[:, 0] # 取出所有背景索引
        ...
        if self.pos_fraction is not None:
            ...
            if len(gt_boxes) == 0 or anchors.shape[0] == 0:
                ...
                labels[bg_inds] = 0 # 将背景anchor的标签设为0
                labels[anchors_with_max_overlap] = gt_classes[gt_inds_force] # 将对应gt类别设为该anchor的标签 有可能gt最大的IoU小于背景阈值
        ...
        if len(gt_boxes) > 0 and anchors.shape[0] > 0:
            ...
            # pdb.set_trace()
            bbox_targets = self.box_coder.encode_torch(gt_boxes[anchor_to_gt_argmax], anchors) # 获取gt_boxes的回归参数
