...
class AnchorHeadRDIoU_3CAT(AnchorHeadTemplate):
    ...
    def get_rdiou(self, bboxes1, bboxes2):
        ...
        # 计算长宽高的比值
        aspect_ratio1 = torch.clamp(l1 / w1, min=1e-8)  # 宽高比
        aspect_ratio2 = torch.clamp(l2 / w2, min=1e-8)
        size_ratio1 = torch.clamp(h1 / (l1 * w1)**0.5, min=1e-8)  # 体积形状比例的一个简化表示
        size_ratio2 = torch.clamp(h2 / (l2 * w2)**0.5, min=1e-8)

        # 计算长宽高比的差异惩罚项
        ar_penalty = 4 / (math.pi**2) * torch.pow(torch.atan(aspect_ratio2) - torch.atan(aspect_ratio1), 2)
        sr_penalty = 4 / (math.pi**2) * torch.pow(torch.atan(size_ratio2) - torch.atan(size_ratio1), 2)

        # 惩罚项系数
        alpha_ar = ar_penalty / (1 - rdiou + ar_penalty + 1e-8)  # 防止分母为零
        alpha_sr = sr_penalty / (1 - rdiou + sr_penalty + 1e-8)

        u = (inter_diag) / c_diag +  alpha_ar * ar_penalty + alpha_sr * sr_penalty
    ...
    def get_rdiou_guided_reg_loss(self):
        ...
        rdiou_loss_n = rdiou - u # 其实这里才是代表了(C/D)IOU
        rdiou_loss_n = torch.clamp(rdiou_loss_n,min=-1.0,max = 1.0)
        rdiou_loss_m = 1 - rdiou_loss_n # Loss = 1 - (C/D)IOU
        ...
        if box_dir_cls_preds is not None:
            ...
            dir_loss = self.dir_loss_func(dir_logits, dir_targets, weights=reg_weights) # 交叉商损失
    ...
    def get_rdiou_guided_cls_loss(self):
        ...
        # 根据cls_targets中每个样本的类别ID，将quality_labels中的值散布（scatter）到one_hot_targets的对应类别位置上 （原本在对应类别位置处的真值为1，但此处替换为anchor与对应gt_box的IoU）
        one_hot_targets.scatter_(-1, cls_targets.unsqueeze(dim=-1).long(), quality_labels.unsqueeze(dim=-1))
        cls_preds = cls_preds.view(batch_size, -1, self.num_class)
        one_hot_targets = one_hot_targets[..., 1:]
        cls_loss_src = self.cls_loss_func(cls_preds, one_hot_targets, weights=cls_weights)  # [N, M] focal loss
